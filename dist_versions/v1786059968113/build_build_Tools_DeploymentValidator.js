...
function validate(cfg){
  ...
  let missingIncludes = [];
  for (let f of buildFiles){ if (!f.endsWith('.html') && !f.endsWith('.js') && !f.endsWith('.gs')) continue; const c = fs.readFileSync(path.join(buildDir,f),'utf8'); let match; includePattern.lastIndex = 0; while ((match = includePattern.exec(c)) !== null){ const inc = match[1]; // should map to a build file base name
